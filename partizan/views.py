...
class HolidayDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Получаем только ОБРАБОТАННЫЕ заявки (processed=True)
        all_bookings = FullOrder.objects.filter(
            selected_date__gte=date.today(),
            processed=True  # Только обработанные заявки занимают слоты
...
def create_full_order(request):
    """Полная заявка - зал бронируется только после обработки админом"""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Метод не поддерживается'})
    ...
    try:
        ...
        phone = request.POST.get('phone', '').strip()
        email = request.POST.get('email', '').strip()  # ДОБАВЬТЕ ЭТУ СТРОКУ
        ...
        # Проверяем доступность ТОЛЬКО по обработанным заявкам
        ...
        order = FullOrder.objects.create(
            ...
            full_name=full_name,
            phone=phone,
            email=email,  # ДОБАВЬТЕ ЭТУ СТРОКУ
        ...
        return JsonResponse({'success': True, 'message': 'Заявка отправлена на рассмотрение! Ожидайте подтверждения администратора.'})
...
def profile_view(request):
    """Личный кабинет с заявками пользователя"""
    # Ищем заявки по email пользователя
    user_orders = FullOrder.objects.filter(
        email=request.user.email
    ).order_by('-created_at') if request.user.email else []
...
def confirm_order(request):
    """Сохранение подтверждения пользователя о приходе"""
    try:
        data = json.loads(request.body)
        order_id = data.get('order_id')
        confirmed = data.get('confirmed')
        
        order = get_object_or_404(FullOrder, id=order_id)
        
        # Добавляем поле confirmed в модель FullOrder
        order.confirmed = confirmed
        order.save()
        
        return JsonResponse({'success': True, 'message': 'Статус подтверждения сохранен'})
...
def cancel_order(request):
    ...
    try:
        ...
        # Можно только отменять необработанные заявки
        if order.processed:
            return JsonResponse({'success': False, 'message': 'Нельзя отменить обработанную заявку'})
